...
import com.bben.common.util.MD5Util;
import com.github.pagehelper.PageHelper;
import com.bben.common.PageInfo;
import com.bben.common.util.U;
import com.bben.medical.mapper.medical.TUserMapper;
...
//    @CacheEvict(key = "#userId")
    ...
    @Override
//    @CachePut(key = "#record.userId", unless = "#record == null")
    @Transactional(isolation = Isolation.READ_COMMITTED, propagation = Propagation.REQUIRED)
    public TUser insertSelective(TUser record) {
        record.setUserCode(CodeUtil.generatUserCode());
        if (U.isNotBlank(record.getPassword())){
            record.setPassword(MD5Util.md5Password(record.getPassword()));
        }
        if (U.isNotBlank(record.getPasswordAnswer())){
            record.setPasswordAnswer(MD5Util.md5Password(record.getPasswordAnswer()));
        }
