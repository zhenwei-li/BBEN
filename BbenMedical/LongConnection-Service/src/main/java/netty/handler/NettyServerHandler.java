...
//                f = NettyChannelMap.get(pingMsg.getClientId()).writeAndFlush(replyPing);   //回写数据给client端
            ...
            case UPGRADE: {
                ...
                f = NettyChannelMap.get(upgradeRequest.getEquipmentType() + ":" + upgradeRequest.getClientId()).writeAndFlush(upgradeResponse);   //回写数据给client端
                f.addListener((ChannelFutureListener) future -> {
                    if (future.isSuccess()) {
            ...
            case VOICE: {
                ...
                f = NettyChannelMap.get(voiceRequest.getEquipmentType() + ":" + voiceRequest.getClientId()).writeAndFlush(voiceRequest);  //回写数据给client端
                f.addListener((ChannelFutureListener) future -> {
                    if (future.isSuccess()) {
            ...
            case CANCEL_VOICE: {
                ...
                f = NettyChannelMap.get(cancelVoiceRequest.getEquipmentType() + ":" + cancelVoiceRequest.getClientId()).writeAndFlush(cancelVoiceRequest);
                f.addListener((ChannelFutureListener) future -> {
                    if (future.isSuccess()) {
            ...
            case VIDEO: {
                ...
                f = NettyChannelMap.get(videoRequest.getEquipmentType() + ":" + videoRequest.getClientId()).writeAndFlush(videoRequest);
                f.addListener((ChannelFutureListener) future -> {
                    if (future.isSuccess()) {
            ...
            case CANCEL_VIDEO: {
                ...
                f = NettyChannelMap.get(cancelVideoRequest.getEquipmentType() + ":" + cancelVideoRequest.getClientId()).writeAndFlush(cancelVideoRequest);
                f.addListener((ChannelFutureListener) future -> {
                    if (future.isSuccess()) {
...
//                msgNoticeResponse.setEquipmentType(msgNoticeRequest.getEquipmentType());
                ...
                if (tmpClientId.length() == 0 || tmpClientId.equals("*")) {
                    ...
                    f = NettyChannelMap.get(msgNoticeRequest.getEquipmentType() + ":" + msgNoticeRequest.getClientId()).writeAndFlush(msgNoticeRequest);  //回写数据给client端
                    f.addListener((ChannelFutureListener) future -> {
                        if (future.isSuccess()) {
